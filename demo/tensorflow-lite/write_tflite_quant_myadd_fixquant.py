...
def create_add_model(output_path):
    ...
    # 创建量化参数对象
    input0_scale_vec = builder.CreateNumpyVector(np.array([0.2], dtype=np.float32))
    input0_zero_point_vec = builder.CreateNumpyVector(np.array([0], dtype=np.int8))
    input1_scale_vec = builder.CreateNumpyVector(np.array([0.1], dtype=np.float32))
    input1_zero_point_vec = builder.CreateNumpyVector(np.array([0], dtype=np.int8))
    output_scale_vec = builder.CreateNumpyVector(np.array([0.5], dtype=np.float32))
    output_zero_point_vec = builder.CreateNumpyVector(np.array([0], dtype=np.int8))

    QuantizationParameters.Start(builder)
    QuantizationParameters.AddScale(builder, input0_scale_vec)
    QuantizationParameters.AddZeroPoint(builder, input0_zero_point_vec)
    input0_quant_params = QuantizationParameters.End(builder)

    QuantizationParameters.Start(builder)
    QuantizationParameters.AddScale(builder, input1_scale_vec)
    QuantizationParameters.AddZeroPoint(builder, input1_zero_point_vec)
    input1_quant_params = QuantizationParameters.End(builder)

    QuantizationParameters.Start(builder)
    QuantizationParameters.AddScale(builder, output_scale_vec)
    QuantizationParameters.AddZeroPoint(builder, output_zero_point_vec)
    output_quant_params = QuantizationParameters.End(builder)
    ...
    Tensor.AddQuantization(builder, input0_quant_params)
    ...
    Tensor.AddQuantization(builder, input1_quant_params)
    ...
    Tensor.AddQuantization(builder, output_quant_params)
    ...
    with flex_builder.Map():
        ...
        flex_builder.Float("input0_scale", 0.2)
        flex_builder.Int("input0_zp", 0)
        flex_builder.Float("input1_scale", 0.2)
        flex_builder.Int("input1_zp", 0)
        flex_builder.Float("output_scale", 0.5)
        flex_builder.Int("output_zp", 0)
